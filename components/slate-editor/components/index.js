...
export const Icon = styled(({className, ...rest}) => {
  ...
  // return <span onClick={onClick} className={`material-icons ${props.className}`} children={props.children}/>
...
export const BlockQuote = styled((props) => {
  return (
    <blockquote className={`${props.className}`} {...props.attributes}> {props.children} </blockquote>
  )
})`
  ...
  return (
    <React.Fragment>
      <div className="prop-image" style={{width: '260px', ...styles[align]}}>
        ...
        <Image style={styles[align]} src={src} selected={isFocused} {...attributes} />
      </div>
    </React.Fragment>
  )
}